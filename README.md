...
* расписание плановой ротации задаётся в веб-интерфейсе в формате cron и работает по локальному времени сервера
...
то есть каждый день в 00:00 по локальному времени сервера). Расписание меняется в веб-интерфейсе
и записывается в `config.json`; старый параметр `cacheCleanInterval` используется только как
совместимый fallback, если cron-расписания нет в конфиге.
Формат cron состоит из 5 полей: `минута час день-месяца месяц день-недели`; поддерживаются `*`,
списки через запятую, диапазоны и шаги вроде `*/15`.
...
Эти же лимиты и cron-расписание ротации можно менять в веб-интерфейсе:

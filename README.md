...
Конвертации выполняются в общей очереди. По умолчанию одновременно работают два задания, ещё 16 могут ожидать; ожидание и выполнение ограничены двумя минутами каждое. При таймауте завершается дерево процессов конвертера. Незавершённый результат не попадает в кэш.

| Параметр config.json | Переменная Docker | По умолчанию |
| --- | --- | --- |
| `conversionConcurrency` | `INPX_CONVERSION_CONCURRENCY` | `2` |
| `conversionQueueLimit` | `INPX_CONVERSION_QUEUE_LIMIT` | `16` |
| `conversionTimeoutMs` | `INPX_CONVERSION_TIMEOUT_MS` | `120000` |
| `conversionQueueTimeoutMs` | `INPX_CONVERSION_QUEUE_TIMEOUT_MS` | `120000` |

Переменные окружения имеют приоритет; изменения применяются после перезапуска. Для тяжёлых книг можно увеличить таймаут. При заполнении очереди приложение возвращает ошибку с предложением повторить запрос позже.
...
Для OPDS действует отдельный от входа в веб-интерфейс лимит неудачных попыток с одного IP: по умолчанию 8 за 15 минут (`loginRateLimitMaxAttempts` / `loginRateLimitWindowMs`). Ответ на превышение — HTTP 429 с `Retry-After`. Запрос без Authorization только получает стандартный Basic Auth challenge и не расходует лимит. Успешный OPDS-вход не сбрасывает предыдущие ошибки, чтобы известный пароль одного профиля не позволял перебирать другие. Метрики различают попытки с `kind="opds"`.
...
Полный ZIP проверяется до изменения данных: формат JSON, ключ шифрования, состав архива и распакованный размер. На время создания и применения бэкапа изменения входящих в него файлов сериализуются. При ошибке записи восстанавливаются прежние файлы; после аварийного завершения незавершённую замену отменяет журнал `.restore-transaction` при следующем запуске. После успешного восстановления сеансы профилей сбрасываются. Для применения всех настроек нужен перезапуск; локальные пути `dataDir`, `tempDir`, `logDir` сохраняются.

Размеры запросов задаются отдельно от скачивания INPX (`maxPayloadSize`):

| config.json | Docker / переменная окружения | По умолчанию | Максимум |
| --- | --- | --- | --- |
| `wsMessageLimitMb` | `INPX_WS_MESSAGE_LIMIT_MB` | 1 MiB | 16 MiB |
| `importLimitMb` | `INPX_IMPORT_LIMIT_MB` | 16 MiB | 64 MiB |
| `backupUploadLimitMb` | `INPX_BACKUP_UPLOAD_LIMIT_MB` | 32 MiB ZIP | 128 MiB |
| `backupExpandedLimitMb` | `INPX_BACKUP_EXPANDED_LIMIT_MB` | 64 MiB | 256 MiB |

Для передачи ZIP через WebSocket учитывается увеличение размера при base64-кодировании. Обычные команды ограничены отдельно; сохранение настроек читалки/профиля допускает 6 MiB, чтобы сохранялись фоновые изображения до 4 MiB. Одновременно выполняется до 8 запросов на соединение и до 64 на сервер; крупное сообщение обрабатывается только одно. Перегрузка закрывает соединение с кодом 1013, превышение транспортного лимита — 1009. Импорт личных списков допускает до 1000 списков и до 100 000 записей книг суммарно.

Новые пароли профилей хранятся как scrypt с индивидуальной случайной солью. Старые SHA-256 хеши автоматически обновляются после успешного входа через веб или OPDS. После обновления хеша смена логина не требует смены пароля; для ещё не обновлённого хеша при смене логина нужно повторно указать пароль. Пароль ограничен 4096 байтами UTF-8.

...
* начиная с `1.6.5`, Windows standalone-архив включает утилиты для обложек (`7zr` и `djxl`), поэтому их не нужно ставить отдельно
...
* для Windows-архива `1.6.5+` дополнительные `7-Zip` и `libjxl` вручную ставить не нужно: нужные инструменты уже лежат рядом с `inpx-web.exe`

...
Этот репозиторий ориентирован на self-hosted использование `inpx-web` с упором на:

* запуск в Docker и Proxmox LXC
* работу с `.7z` / `fLibrary` коллекциями
...
В этом форке есть три разных сценария запуска, и их важно не путать:

1. `Docker / сервер` — основной и рекомендуемый сценарий этого репозитория.
   Подходит для домашнего сервера, NAS, unRAID, Proxmox LXC, VPS и постоянного web-доступа к библиотеке.
2. `Standalone-бинарник` — дополнительный сценарий для локального запуска без Docker.
   Удобен, если нужен один исполняемый файл для Windows, Linux или macOS.
3. `Запуск через Node.js` — вариант для разработки, отладки или ручного локального старта без Docker и без готового бинарника.

Коротко:

* если хотите запустить каталог дома или на сервере — идите в раздел Docker
* если хотите просто скачать готовый исполняемый файл — используйте релизы GitHub
* если хотите разрабатывать или запускать проект вручную — используйте Node.js запуск ниже

## Важно про этот форк

Этот README описывает именно форк `AceAsket/inpx-web`, а не только исходный проект.

Что это означает на практике:

* основной путь здесь — self-hosted запуск в Docker
* standalone-бинарники тоже поддерживаются, но это дополнительный режим, а не центральный сценарий
* часть старых разделов ниже сохранена для совместимости с исходной документацией, но в этом форке они приведены в порядок под текущую структуру проекта
* если вам нужен именно “классический локальный запуск одним бинарником”, ориентируйтесь на разделы про релизы и standalone-запуск, а не на Docker-инструкции
...
* [Локальная FB2-читалка для отладки](#локальная-fb2-читалка-для-отладки)
* [Внешние источники витрин](#внешние-источники-витрин)
* [Обратная связь](#обратная-связь)

## Ключевые отличия

Этот форк развивает оригинальный `inpx-web` в сторону удобного self-hosted каталога для больших `fLibrary` / `.7z`-коллекций, Docker-развёртывания и более современной встроенной читалки.
...
- поддерживается сборка релизов под Linux, MacOS и Windows

<a id="usage" />

## Использование
Основной и самый простой способ запуска для этого форка — Docker.

Если в GitHub Release приложены готовые бинарные архивы для вашей платформы, распакуйте архив и поместите приложение `inpx-web` в папку с `.inpx`-файлом и файлами библиотеки, затем запустите его.

Если в релизе есть только исходники, используйте Docker-сценарий из разделов ниже или локальную сборку из исходников.
...
Этот блок относится только к локальному запуску без Docker.

Он нужен, если вы хотите:

* скачать готовый бинарник из GitHub Releases
* запускать `inpx-web` как обычный `.exe` / исполняемый файл
* работать с библиотекой локально, без контейнера

Если вы поднимаете `inpx-web` как домашний или серверный сервис, этот раздел можно пропустить и использовать Docker-инструкции выше.

Внутри standalone-сценария есть два варианта:

1. `Готовый бинарник из релиза`
2. `Запуск через Node.js из исходников`
...
Этот раздел нужен только если вы хотите собрать standalone-бинарники вручную.

Для большинства пользователей он не нужен:

* для сервера используйте Docker
* для локального запуска без сборки используйте готовые assets из GitHub Releases

Сборка выполняется в Linux-среде.
...
Если Docker не нужен, проект можно запускать напрямую через Node.js.

Это подходит для:

* локальной отладки
* ручного запуска на рабочей станции
* сценария, где вы не хотите собирать отдельный бинарник
...
### Внешние источники витрин

Во вкладках `Новинки`, `Популярное` и во внешнем источнике можно использовать не только локальные данные библиотеки, но и внешние веб-витрины.

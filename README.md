...
Плановая ротация привязана к локальному времени сервера: при интервале `1440` минут она запускается
на ближайшей серверной полуночи, при `60` минутах — на ближайшем начале часа.
...
-e INPX_BOOK_CACHE_SIZE_MB=2048
-e INPX_COVER_CACHE_SIZE_MB=512
-e INPX_CACHE_CLEAN_INTERVAL_MINUTES=1440
-e INPX_CACHE_CLEAN_TARGET_RATIO=0.8
```

Эти же лимиты можно менять в веб-интерфейсе: `Настройки` → `Администрирование` → `Ротация кэша`.
...
```js
{
    ...
    // запуск привязан к локальному времени сервера: 1440 - ближайшая полночь, 60 - начало часа
    // 0 - отключить чистку

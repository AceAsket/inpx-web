...
Форк [`bookpauk/inpx-web`](https://github.com/bookpauk/inpx-web) для self-hosted библиотек с упором на большие `.7z` / `fLibrary` коллекции, Docker-развёртывание и обновлённую встроенную FB2-читалку. README ниже в первую очередь описывает сценарий `Docker / сервер`, но также покрывает standalone-бинарники и запуск из исходников через Node.js.

Ключевое в этом форке:

* запуск в Docker и Proxmox LXC, работа с большими `.7z` / `fLibrary` коллекциями и конвертацией через Calibre
...
* отправка и конвертация книг в `epub`, `mobi`, `pdf`, а также отправка через Telegram и email
